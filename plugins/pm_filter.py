    ...
    # don't change anything without contacting me @kingvj01
        ...
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('ᴅɪsᴄʟᴀɪᴍᴇʀ', url="https://telegra.ph/DISCLAIMER-11-20-32")
            ]]
        else:
            buttons = [[
                InlineKeyboardButton('Hᴇʟᴘ 🔧', callback_data="help"),
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if imdb:
        ...
        if not settings["button"]:
            cap+="<b>\n<u>I Fᴏᴜɴᴅ Sᴏᴍᴇ Rᴇꜱᴜʟᴛꜱ Fᴏʀ Yᴏᴜʀ Qᴜᴇʀʏ : {search}</u></b>\n\n"
            for file in files:
                cap += f"<b>\n📎 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file['file_id']}'>[{get_size(file['file_size'])}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file['file_name'].split()))}\n</a></b>"
    else:
        if settings["button"]:
            cap = f"<b>ʜᴇʏ {message.from_user.mention}</b>"
        else:
            cap = f"<b>ʜᴇʏ {message.from_user.mention}</b>"
            cap+="<b>\n<u>I Fᴏᴜɴᴅ Sᴏᴍᴇ Rᴇꜱᴜʟᴛꜱ Fᴏʀ Yᴏᴜʀ Qᴜᴇʀʏ : {search}</u></b>\n\n"
            for file in files:
